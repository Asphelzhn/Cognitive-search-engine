# encoding: utf-8
'''@author: Edward & Severn @file: WeightingModule.py @time: 2019/11/12
22:05 @desc: This is the class for weighting module. This module receives
the list of search results from Elastic Search, calculate a weight for each
based on a set of factors, sort the list, and return that sorted list. The
"set of factors" that influence the weight of a document are preliminarily:
ES-Score（Elastic Search results） Keyword similarity (compilation of the
...
class WeightingModule:
    ...
    def calculate_keyword_similarity(elastic_search_results, search_query):
        ...
        for title in elastic_search_results:
            ...
            name_weight_set = (models.Pdf_Name_Keyword_Weight
                               .objects.filter(pdf_name=title))
    ...
    def calculate_popularity(elastic_search_results):
        ...
        for popularity in popularity_score:
            normalize_popularity_score.append(
                WeightingModule.normalize(popularity, min_score, max_score))
    ...
    def calculate_score_after_weight(elastic_search_results, search_query):
        ...
        for result in elastic_search_results:
            initial_score = WeightingModule.normalize(
                length, 0, len(elastic_search_results))
        ...
        for similarity in similarity_score_list:
            normalize_similarity_score_list.append(WeightingModule.normalize(
                similarity, min_score, max_score))
        ...
        popularity_score_list = WeightingModule.calculate_popularity(
            elastic_search_results)
        index = 0
        for result in elastic_search_results:
            score_after_popularity_weight = (weight_popularity
                                             * popularity_score_list[index])

            score_dict[result] += score_after_popularity_weight
            index += 1

        sorted_document_list = sorted(score_dict.items(),
