...
class imp_sent_api():
    ...
    def generateAbstract(self, query, impsentenceList, word_frequencies):
        # Här skulle jag vilja få in datavasObject samt word frequencies
        # för ett visst dokument
        size = 4
        abstract = imp_sent_creator.generateAbstract(query,
                                                     impsentenceList,
                                                     size,
                                                     word_frequencies)
        return abstract
...
if __name__ == '__main__':
    ...
    sentences, word_frequencies = s.upload_find_relevant_sentences(
        [test_imp_sent_creator.document1])
    t0 = time.time()
