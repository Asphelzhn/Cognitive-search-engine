# Implementation of rake_algorithm - Rapid Automatic Keyword Extraction
# algorithm as described in: Rose, S., D. Engel, N. Cramer, and W. Cowley (
# 2010). Automatic keyword extraction from individual documents. In M. W.
# Berry and J. Kogan (Eds.), Text Mining: Applications and Theory.unknown:
# John Wiley and Sons, Ltd.
#
# NOTE: The original code (from https://github.com/aneesha/RAKE) has been
# extended by a_medelyan (zelandiya) with a set of heuristics to decide
# whether a phrase is an acceptable candidate as well as the ability to set
# frequency and phrase length parameters important when dealing with longer
# documents
#
# NOTE 2: The code published by a_medelyan (
# https://github.com/zelandiya/RAKE-tutorial) has been additionally extended
# by Marco Pegoraro to implement the adjoined candidate feature described in
# section 1.2.3 of the original paper. Note that this creates the need to
# modify the metric for the candidate score, because the adjoined candidates
...
def load_stop_words(stop_word_file):
    ...
    words @param stop_word_file Path and file name of a file containing stop
    words. @return list A list of stop words.
    """
    stop_words = []
...
def separate_words(text, min_word_return_size):
    ...
    greater than a specified number of characters. @param text The text that
    must be split in to words. @param min_word_return_size The minimum no of
    characters a word must have to be included.
    """
...
#
# Function that extracts the adjoined candidates from a list of sentences
# and filters them by frequency
#
def extract_adjoined_candidates(sentence_list, stoplist,
                                min_keywords, max_keywords,
                                min_freq):
    adjoined_candidates = []
    for s in sentence_list:
        # Extracts the candidates from each single sentence and adds them to
        # the list
        adjoined_candidates += adjoined_candidates_from_sentence(s, stoplist,
                                                                 min_keywords,
                                                                 max_keywords)
...
def adjoined_candidates_from_sentence(s, stoplist, min_keywords, max_keywords):
    ...
    for num_keywords in range(min_keywords, max_keywords + 1):
        ...
        for i in range(0, len(sl) - num_keywords):
            # Position i marks the first word of the candidate. Proceeds
            # only if it's not a stopword
            if sl[i] not in stoplist:
                candidate = sl[i]
                # Initializes j (the pointer to the next word) to 1
                j = 1
                # Initializes the word counter. This counts the
                # non-stopwords words in the candidate
                keyword_counter = 1
                contains_stopword = False
                # Until the word count reaches the maximum number of
                # keywords or the end is reached
                while keyword_counter < num_keywords and i + j < len(sl):
                    # Adds the next word to the candidate
                    candidate = candidate + ' ' + sl[i + j]
                    # If it's not a stopword, increase the word counter. If
                    # it is, turn on the flag
                    ...
                    # Next position
                    j += 1
                # Adds the candidate to the list only if: 1) it contains at
                # least a stopword (if it doesn't it's already been
                # considered) AND 2) the last word is not a stopword AND 3)
                # the adjoined candidate keyphrase contains exactly the
                # correct number of keywords (to avoid doubles)
                if (contains_stopword and candidate.split()[-1] not in stoplist
...
#
# Function that filters the adjoined candidates to keep only those that
# appears with a certain frequency
#
def filter_adjoined_candidates(candidates, min_freq):
    # Creates a dictionary where the key is the candidate and the value is
...
def generate_candidate_keywords(sentence_list, stopword_pattern,
                                stop_word_list, min_char_length=1,
                                max_words_length=5, min_words_length_adj=1,
                                max_words_length_adj=1, min_phrase_freq_adj=2):
    phrase_list = []
    for s in sentence_list:
        ...
        for phrase in phrases:
            ...
            if phrase != "" and is_acceptable(phrase,
                                              min_char_length,
                                              max_words_length):
...
class Rake(object):
    def __init__(self, stop_words_path, min_char_length=1,
                 max_words_length=5, min_keyword_frequency=1,
                 min_words_length_adj=1, max_words_length_adj=1,
...
if test and __name__ == '__main__':
    text = "Compatibility of systems of linear constraints over the set of " \
           ...
           "in solving all the considered types of systems and systems of " \
           "mixed types. "

    # Split text into sentences
    sentenceList = split_sentences(text)
    # stoppath = "FoxStoplist.txt" Fox stoplist contains "numbers",
    # so it will not find "natural numbers" like in Table 1.1
    stoppath = "data/stoplists/SmartStoplist.txt"  # SMART stoplist misses
    # some of the lower-scoring keywords
