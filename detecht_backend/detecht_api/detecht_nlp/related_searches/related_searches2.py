...
# numOfExtraWords < 3
def queryDatabase(querySplit, numberOfExtraWords, numOfQueries, textsThatCantBeUsed):
    ...
    result = Search_Autocomplete.objects.filter(n_gram__istartswith=tempQuery)
...
def generateRelatedSearches(text):
    ...
    oneWordDeletePlusOne = generate_one_new_word(listDelete,resultsToBeGenerated,textsThatCantBeUsed)
    ...
    oneWordDeletePlusTwo = generate_two_new_word(listDelete,resultsToBeGenerated,textsThatCantBeUsed)
    if oneWordDeletePlusTwo != 0:
        ...
        if len(oneWordDeletePlusTwo)==resultsToBeGenerated:
            ...
            resultsToBeGenerated=resultsToBeGenerated-len(oneWordDeletePlusTwo)
    else:
        resultsToBeGenerated=resultsToBeGenerated+1

    oneWordAdded = generate_one_new_word(list,resultsToBeGenerated,textsThatCantBeUsed)
    if oneWordAdded !=0:
