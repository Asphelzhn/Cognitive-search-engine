...
# This is example of how to add new row into database.
# If there is no user_id, just set id as null.
def add(query, id):
    standardized_query = standardize_query(query)
    date = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
    search = Searches_Database()
    models.Searches_Database.objects.create(user_id=id,search_date=date,search_query=query,standardized_search_query=standardized_query,search_score=2)
